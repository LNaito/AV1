...
export class PecaCLI {
    ...
    static carregarPecas(): void {
        try {
            ...
            this.pecas = dados.map((dado: any) => new Peca(dado.nome, dado.tipo, dado.fornecedor, dado.status));
    ...
    static async cadastrarPeca(): Promise<void> {
        const dados = await inquirer.prompt([
            ...
            { type: 'list', name: 'tipo', message: 'Qual o tipo da peça? ', choices: Object.values(TipoPeca) },
            { type: 'input', name: 'fornecedor', message: 'Qual o fornecedor da peça? ' },
            { type: 'list', name: 'status', message: 'Qual o status inicial da peça? ', choices: Object.values(StatusPeca) }
        ]);

        const peca = new Peca(dados.nome, dados.tipo, dados.fornecedor, dados.status);
    ...
    static async buscarPorNome(): Promise<void> {
        const { nome } = await inquirer.prompt([{ type: 'input', name: 'nome', message: 'Digite o nome da peça: ' }]);
    ...
    static async atualizarStatusPeca(): Promise<void> {
        const { nome } = await inquirer.prompt([{ type: 'input', name: 'nome', message: 'Digite o nome da peça para atualizar status: ' }]);
        const peca = this.buscarPecaPorNome(nome);
        if (peca) {
            const { novoStatus } = await inquirer.prompt([{ type: 'list', name: 'novoStatus', message: 'Selecione o novo status:', choices: Object.values(StatusPeca) }]);
    ...
    static async show(): Promise<void> {
        ...
        while (!sair) {
            ...
            switch (acao) {
                case 'Cadastrar Nova Peça': await this.cadastrarPeca(); break;
                case 'Listar Todas Peças': await this.listarPecas(); break;
                case 'Buscar Peça por nome': await this.buscarPorNome(); break;
                case 'Atualizar Status da Peça': await this.atualizarStatusPeca(); break;
                case 'Salvar Peças': this.salvarPecas(); break;
                case 'Carregar Peças': this.carregarPecas(); break;
